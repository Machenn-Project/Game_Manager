# Tomoro — Azure-backed game version & CDN manager
...
{container}/
  {gameName}/
    ...
    version.json                  <- stable "what's current" manifest for launchers; same path forever
    current/
...
- **Executable name** (optional, entered on upload): the main executable's filename (e.g. `Game.exe`). Used to pick which uploaded file `version.json`'s `url` points at; if left blank, it falls back to the first current file. It carries forward from the full build to its patches unless overridden.
...
- Deleting a version removes every file under that `versions/{versionId}/` path and updates the manifest (and the current mirror/version.json, if that version fed them); deleting a game removes everything under `{gameName}/`, including the mirror and version.json.

## 5. version.json (for launchers/auto-updaters)

A permanent `{gameName}/version.json` is rewritten every time the current version changes (new full build, new patch, or a deletion that changes what's current). Point your game launcher at its **Copy version.json link** URL (top of the game panel) and poll it to check for updates:

```json
{
  "version": "1.4.1-patch2",
  "url": "https://.../current/Game.exe",
  "gameName": "MyGame",
  "description": "Fixes the crash on level 3.",
  "executableName": "Game.exe",
  "timestamp": "2026-09-18T10:19:00.000Z"
}
```

If there's no current version yet (no full build uploaded), `version.json` doesn't exist.
...
Set `APP_PASSWORD` in `.env` to lock the app down:

- Opening the app shows a lock screen — you need the password to view or manage anything (a session token is issued and kept in the browser tab's `sessionStorage`, valid for 12 hours).
- Every create/upload/delete (new game, new version/patch, deleting a game/version/file) asks for the password again to confirm, even if the app is already unlocked.
- A **Lock** button appears at the bottom of the sidebar to manually re-lock the app.
- Leave `APP_PASSWORD` empty to disable the gate entirely (the old, open-by-default behavior).

This is a single shared password, not per-user accounts — it's meant to keep casual/unauthorized access out, not to replace a real auth system.

## 8. Notes on going to production

- For anything beyond a single shared password (e.g. per-user accounts, audit trails), put the app behind your existing login system or a VPN instead.
